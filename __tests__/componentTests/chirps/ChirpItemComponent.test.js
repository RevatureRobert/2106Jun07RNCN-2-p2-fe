...
import  { Image, Text, Pressable, TouchableOpacity } from 'react-native';
...
import { diveLikeCrazy } from '../../../src/shared/functions';
...
describe('Testing ChirpItemComponent', () => {
    ...
    beforeEach( () => {
        ...
        shallowWrapper = shallow(nestedHell(store))
    });
    ...
    it('should display the timestamp of the chirp', () => {
      const userWrapper = wrapper.findWhere( w => 
        w.text().includes(new Date(Number(timestamp)).toLocaleString())
      );
      expect(userWrapper.length > 0).toBe(true);
    });
    ...
    it('user can press the chirp', () => {
      ...
      */
      ...
      const wrap = shallow(temp.length < 2 ? temp : temp.get(0));
